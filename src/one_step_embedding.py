...
from tensorflow.python.keras import models, layers, optimizer_v1
from tensorflow.python.keras.preprocessing.sequence import pad_sequences
from torch import nn

from src.basic import GeneralLogger
from src.config import DATA_PATH_PLANS_FOR_TRAIN, DATA_PATH_TXT
...
class EmbeddingNet4Nodes:
    ...
    def fit(self, x: np.array, y: np.array):
        self.model.fit(x, y, validation_split=0.2, epochs=100, batch_size=128, shuffle=True)
...
class Embedding:
    ...
    def load_from(_query_plan_tree: QueryPlan = None,
        ...
        return _query_plan_tree

    @staticmethod
    def get_data_for_node_embedding(_query_plan_dir_for_train: str) -> tuple[
        ...
        plan_list = list()
        nodes_list = list()
        for file_name in os.listdir(_query_plan_dir_for_train):
            node_list = list()
            with open(os.path.join(_query_plan_dir_for_train, file_name)) as query_plan_file:
        ...
        # #
        with open(os.path.join(DATA_PATH_TXT, "all_sentences_of_nodes.txt"), "w", encoding="utf-8") as txt_f:
            for _vector in nodes_list:
                txt_f.write("|".join(_vector) + "\n")
        # # 拿到处理后的数据
        ...
        # data_y = np.array(map(lambda cost: np.log(float(cost)), cost_labels))
    ...
    def vectorize_sentence(sentence: list[str], vocab_encode_mapping: dict):
        return list(map(
            lambda word: vocab_encode_mapping[word].reshape(len(vocab_encode_mapping), ),
            sentence)
        )
    ...
    def flow(self):
        # data_x, data_y, plan_list, vocab_encode_mapping, *_ = self.get_data_for_node_embedding(self.query_plan_dir_for_train)
        # EmbeddingLogger.info(f"data X shape: ({data_x.shape}")
        # EmbeddingLogger.info(f"data y shape: ({data_y.shape}")
        # pickle.dump(plan_list, open("plan_list", "wb"))
        # pickle.dump(vocab_encode_mapping, open("vocab_encode", "wb"))
        # net = EmbeddingNet4Nodes(input_shape=data_x.shape[1:])
        # net.model.summary()
        # net.fit(data_x, data_y)
        # net.save_model("embedding_all_nodes", ".")
        # EmbeddingLogger.info("embedding over")
        # embedding时取出倒数一层的输出

        plan_list = pickle.load(open("plan_list", "rb"))
        vocab_encode_mapping = pickle.load(open("vocab_encode", "rb"))
        print(*plan_list[0], sep='\n')
        net = EmbeddingNet4Nodes(load_from="embedding_all_nodes-1670072035404785400.h5")
        net.model.summary()
        ...
        # # ((113, 35, 64) 和 ((113,)
        np.save("cost_learner_x", plan_sequences)
        np.save("cost_learner_y", root_cost_labels)
