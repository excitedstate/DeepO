...
class CostEstimationTrainer:
    ...
    def __init__(self, _fp_plan_sequences_output, _fp_cost_labels, _fp_std_scalar_labels):
        ...
        self.sequences, self.cost_labels = self.load_data(
            _fp_plan_sequences_output,
            _fp_cost_labels)
        self.x_train, self.y_train, self.x_test, self.y_test = self.preprocess(_fp_std_scalar_labels)
    ...
    def evaluate(self):
        ...
        pred, pred_vals = self.pred_cost(self.x_test[idx].unsqueeze(0), self.sc, self.net)

        unscaled_y, in_range, y_under_upper, y_above_lower = self.evaluate_pred_vals(pred_vals, self.y_test[idx], 2, self.sc)
    ...
    def pred_cost(x, sample_nbr=100, _sc: MinMaxScaler = None, _net: nn.Module = None):
        ...
        pred_vals = _sc.inverse_transform(pred_vals)
