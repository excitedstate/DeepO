...
from src.embedding import Node

HintGeneratorLogger = GeneralLogger(name="hint_generator", stdout_flag=True, stdout_level=logging.WARNING,
                                    file_mode="w")
...
class CostEstimator:
    ...
    def flow(self, plan_dir, leaf_model_path, fp_column_statistics):
        max_length = 8
    ...
    def leaf_embedded(self, plan, model_path, fp_column_statistics):
        ...
        vocab_dict = self.get_vocabulary_encoding()
        ...
        padded_sentences = self.padding_sentence(test_data)
        # load model
    ...
    def get_vocabulary_encoding(self):
    ...
    def pred_cost(x, sample_nbr=100, _sc=None, _net=None):
        pred_vals = [_net(x).cpu().item() for i in range(sample_nbr)]
