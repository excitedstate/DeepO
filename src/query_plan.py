...
class CostInfo:
    ...
    def parse_content(self):
        ...
        self.estimate_start_cost, self.estimate_end_cost, self.estimate_rows_involved, \
            ...
            self.content).groups() if '(never executed)' not in self.content and self.content.count('(') > 1 else (
...
class Operator:
    ...
    def _do_Sort(self, _additional_info_lines: list[str]):
        """
          ->  Sort  (cost=52278.44..52379.08 rows=40257 width=4) (actual time=2962.847..3852.427 rows=36109 loops=2)
            Sort Key: t.id
            Sort Method: quicksort  Memory: 3058kB
            Worker 0:  Sort Method: quicksort  Memory: 3058kB
        """
        if len(_additional_info_lines) > 0 and _additional_info_lines[0].startswith("Sort Key: "):
            self.op_on = [Operator.remove_prefix(_additional_info_lines[0], "Sort Key: ")]
            # # we choose to believe

    def _do_MergeJoin(self, _additional_info_lines: list[str]):
        """
             ->  Merge Join  (cost=1610.44..161011.30 rows=2860347 width=0) (actual time=3669.287..399448.999 rows=2148084 loops=1)
                    Merge Cond: (t.id = mk.movie_id)
        """
        if len(_additional_info_lines) > 0 and _additional_info_lines[0].startswith("Merge Cond: "):
            self.parse_index_recheck_join_cond_exp(Operator.remove_prefix(_additional_info_lines[0], "Merge Cond: "))

    def _do_BitMapAnd(self, _additional_info_lines: list[str]):
        """
        ->  BitmapAnd  (cost=10.64..10.64 rows=1 width=0) (actual time=0.148..0.148 rows=0 loops=18064)
          ...
        Args:
            _additional_info_lines: 应该是一个空列表
    ...
    def _do_IndexOnlyScan(self, _additional_info_lines: list[str]):
        ...
        """
        if len(_additional_info_lines) > 0 and _additional_info_lines[0].startswith("Index Cond: "):
            self.parse_index_recheck_join_cond_exp(Operator.remove_prefix(_additional_info_lines[0], "Index Cond: "))
    ...
    def _do_IndexScan(self, _additional_info_lines: list[str]):
        ...
        if len(_additional_info_lines) > 0 and _additional_info_lines[0].startswith("Index Cond: "):
            ...
            ParseQueryPlanLogger.debug(f"Index Scan no addition_info_lines, {self.content}")
...
class QueryPlanNode:
    ...
    def __str__(self):
        return f'QueryPlanNode<{self.node_type.name}>[{self.operator}]: id: {self.id}, layer: {self.layer}, ' \
               f'parent: {self.parent.id if self.parent is not None else None}'
...
class QueryPlan:
    ...
    def parse_content(self):
        ...
        if not self.EVAL_MODE:
            try:
                ...
                ParseQueryPlanLogger.debug(f"query plan raw error: {e}, no time output")
                ParseQueryPlanLogger.debug(gene[-1])
    ...
    def post_order(self, _callback: typing.Callable[[QueryPlanNode, typing.Any, ...], typing.NoReturn] = None,
        ...
        def __post_order(_cur_node: QueryPlanNode):
            if (_cur_node or self.EVAL_MODE) and _cur_node is not None:
