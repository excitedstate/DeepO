...
def get_data_and_label(column_min_max_vals, plan_path):
    ...
    for i in range(len(plan)):
        if ("Seq Scan" in plan[i]):
            ...
            if (i + 1 < len(plan) and "actual" not in plan[i + 1] and "Plan" not in plan[i + 1] and "->" not in plan[
                i + 1]):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--plan-dir", type=str, help="path of plan needed to be optimized",
                        default="../example/optimized plans")
    parser.add_argument("--leaf-embedding-path", type=str, help="model path for leaf embedding",
                        default="../model/embedding_model.h5")
    parser.add_argument("--cost-model-path", type=str, help="model path for TreeLSTM",
                        default="../model/cost_model_50_minmax")
